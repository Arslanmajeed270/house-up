(function($) {
    ...
    var Door BudsMarkerImage = {
        url: 'images/Door Buds-marker.png',
        size: new google.maps.Size(47, 47),
        origin: new google.maps.Point(0, 0),
        anchor: new google.maps.Point(24, 21),
        scaledSize: { width: 47, height: 47 }
        ...
        switch (type) {
            ...
            case 'Door Buds':
                poiMarker = new google.maps.Marker({
                    map: map,
                    position: placeLoc,
                    icon: Door BudsMarkerImage,
                });
                Door BudsMarkers.push(poiMarker);
                break;
            case 'shopping':
                poiMarker = new google.maps.Marker({
                    map: map,
    ...
    setTimeout(function() {
        if($('#pxp-sp-map').length > 0) {
            ...
            $('.pxp-sp-pois-nav-Door Buds').click(function() {
                var this_ = $(this);
                if($(this).hasClass('pxp-active')) {
                    $(this).removeClass('pxp-active');
    
                    tooglePOIs(map, Door BudsMarkers);
                } else {
                    $(this).addClass('pxp-active');
    
                    getPOIs(center, map, 'Door Buds');
                    tooglePOIs(map, Door BudsMarkers);
                }
                google.maps.event.addListener(map, 'bounds_changed', function() {
                    if(this_.hasClass('pxp-active')) {
                        getPOIs(map.getCenter(), map, 'Door Buds');
                    }
                });
            });
